...
<br><font style="background:yellow">弱引用对象与软引用对象的最大不同</font>就在于，当GC在进行回收时，需要通过算法检查是否回收软引用对象，而对于弱引用对象，GC总是进行回收。<font style="background:yellow">弱引用对象更容易、更快被GC回收</font>。
